...
module SwisseldexAumImmo
  class UploadMessageBuilding
    # Eidgenoessischer Gebaeudeidentifikator according to specification of BFS
    attr_accessor :egid
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwisseldexAumImmo::UploadMessageBuilding` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwisseldexAumImmo::UploadMessageBuilding`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [egid, address_line1, address_line2, street, house_number, dwelling_number, town, swiss_zip_code].hash
    end
